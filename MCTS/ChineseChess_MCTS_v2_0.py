# 使用 MCTS 算法实现中国象棋的自动走棋，计划加入剪枝操作，未完成
...
class MCTS:
    ...
    def search(self):
        ...
        for _ in range(self.iterations):
            ...
            result_cal = 0.0
            for _ in range(self.iterations*10):  # 模拟次数
                result = self.root.simulate()
                result_cal += result
            result = result_cal / (self.iterations*10)
    ...
    def all_expend(self):
        ...
        while True:
            ...
            while self.root.children and len(self.root.children) == legal_moves_count:
                layer += 1
                if layer == layer_m:  # 层数达到layer_m-1层
                    break
                self.root = self.root.select_child()
            ...
            if not len(self.root.children) == legal_moves_count:
                expend_time_start = time.time()
                child_node = self.root.expand()
                expend_time_end = time.time()
                print('扩展用时{}秒'.format(expend_time_end - expend_time_start))
                if child_node:
                    self.root = child_node
                else:
                    break
            ...
            while self.root.parent:
                ...
                self.root = self.root.parent
            self.root.visits += 1
        all_expend_end = time.time()
        print('所有子节点已扩展完成，用时{}秒'.format(all_expend_end - all_expend_start))
